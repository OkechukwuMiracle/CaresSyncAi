...
router.post('/submit', async (req, res) => {
  try {
    ...
    const { reminder_id, response_text } = req.body;

    console.log('Extracted values:', { reminder_id, response_text });

    if (!reminder_id || !response_text) {
      console.log('Validation failed:', { 
        has_reminder_id: !!reminder_id, 
        has_response_text: !!response_text 
      });
      return res.status(400).json({ error: 'Reminder ID and response text are required' });
    }
    ...
    if (reminderError || !reminder) {
      console.log('Reminder not found:', reminderError);
      return res.status(404).json({ error: 'Reminder not found' });
    }
